...
def generate_obs(time_step,obs_num,varR,N,obs_value):
...
if __name__=='__main__':
    ...
    args_exe=os.path.join(root_directory,'ogs')
    para=os.path.join(root_directory,'water_gas')
    time_step=20
    Nod_num=1681
    N=200
    varR=150.0
...
#    finename_Pressure='water_gas_PRESSURE1.direct'
    ...
    np.savetxt(os.path.join(root_directory,'para_true.txt'),kl_true)
    Fi=KLexpansion(m,n,x,y,deltax,deltay,sigma)
    para_true=np.log(ki_mean)+np.squeeze(np.dot(Fi[:,:kl_term],kl_true))
    np.savetxt(os.path.join(root_directory,'para_true_up.txt'),para_true)
    ...
    p=subprocess.Popen((args_exe,para),stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    p.communicate()
    p.wait()
    time.sleep(1)
    read_trueobs(time_step,obs_Num,Nod_num,root_directory,filename_result)
